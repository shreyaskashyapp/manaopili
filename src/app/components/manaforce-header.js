      ...
      <div className="container mx-auto md:px-10 px-4 py-2">
        <nav className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center gap-6">
            <Link href="/manaforce" className="flex items-center gap-3 fill-white h-[20px]">
            ...
            <Link
              href="/"
            >
              <Button className="text-white border border-white font-sans bg-transparent hover:border-[#455cff] hover:bg-[#455cff] hover:text-white text-sm flex items-center gap-2 rounded-lg px-4 transition-colors">
                Mana&apos;o Pili <ArrowRight className="w-3.5 h-3.5" />
              </Button>
            </Link>
          </div>

          {/* Desktop nav */}
          <div className="hidden lg:flex items-center gap-1 px-10">
            {navItems.map((item) => (
              <NavigationMenu key={item.label}>
                <NavigationMenuList>
                  <NavigationMenuItem>
                    <NavigationMenuTrigger className={cn(
                      "bg-transparent font-sans text-zinc-300 hover:text-[#455CFF] hover:bg-transparent",
                      "data-[state=open]:text-[#455CFF] data-[state=open]:bg-transparent",
                      "text-sm uppercase tracking-wide h-9 px-3 font-normal"
                    )}>
                      {item.label}
                    </NavigationMenuTrigger>
                    <NavigationMenuContent>
                      <div className="w-60 rounded-xl border border-zinc-800/60 bg-[#0a0a14]/80 backdrop-blur-md shadow-xl overflow-hidden p-1">
                        {item.links.map((link) => (
                          <NavigationMenuLink asChild key={link.href}>
                            <Link href={link.href} className="block font-sans px-4 py-2.5 text-sm text-zinc-400 hover:text-[#455CFF] hover:bg-[#455CFF]/5 transition-colors rounded-lg">
                              {link.label}
                            </Link>
                          </NavigationMenuLink>
                        ))}
            ...
            <Link href="/contact">
              <Button className="text-white border border-[#455cff] font-sans bg-[#455cff] hover:border-[#455cff] hover:bg-[#455cff]/70 hover:text-white text-sm flex items-center gap-2 rounded-lg px-4 transition-colors">
                Contact Us
              </Button>
            </Link>
          </div>
