...
export default function Founders({ team }) {
    return (
        <div className="flex flex-col px-10 md:flex-row items-center justify-center gap-24 py-10">
            {team.map((member, index) => (
                <Card className="flex flex-col items-center bg-zinc-900  shadow-lg border-none " key={index}>
                    <CardContent className="relative h-64 w-full rounded-t-lg p-0 ">
                        ...
                        <Image src={member.imagePath} alt="Founders image" className="rounded-t-lg h-64 w-full object-contain" width={400} height={600} />
                    </CardContent>
