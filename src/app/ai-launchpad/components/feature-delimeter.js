    ...
    return (
        <section className="mx-auto py-10">
            ...
            <div className="mb-10 text-white text-center">
                <h1 className="font-heading text-3xl md:text-5xl font-light leading-tight text-balance mb-1">{data?.hero?.title ?? ""}</h1>
                <h2 className="font-heading text-2xl md:text-4xl font-light leading-tight text-balance text-zinc-400">{data?.hero?.subtitle ?? ""}</h2>
            </div>

            {/* Pain Points with Left Accent Bar */}
            <div className="flex flex-col md:flex-row justify-between gap-8 pt-4 group">
                {data?.painPoints?.map((point, index) => (
                    <div key={point?.id ?? index} className="flex gap-6 flex-1 md:border-none border-b pb-4 border-[#455CFF]/20">
                        <div className="flex-1 cursor-default hover-trigger">
                            <div className="h-full md:p-8 relative border-l-2 border-transparent hover:border-[#455CFF] transition-all duration-300">
